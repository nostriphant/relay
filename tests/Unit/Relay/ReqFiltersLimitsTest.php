...
use nostriphant\Relay\Incoming\Req\Limits;
...
it('has a maximum number of filters per subscription.', function () {
    ...
    expect($limit)->toHaveState(rejected: ['max number of filters per subscription (1) reached']);
});
...
it('has a maximum number of filters per subscription. Defaults to 10.', function () {
    ...
    expect($limit)->toHaveState(rejected: ['max number of filters per subscription (10) reached']);
});
